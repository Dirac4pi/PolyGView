...
generated by them contain limited info (compared to their output files), but
most issues can still be resolved by analyzing them with `gview.py`. `gview.py`
visualizes energy and displacement of the trajectory and, when used in
conjunction with `mdf.py` to analyze the positional fluctuations of individual
atoms in the trajectory, is well-suited for studying geometric optimization,
potential surface scans, and comparisons of local minima across potential
surfaces, etc.<br>
...
`$PATH`, gview.py can invoke it to model 2D structures based on ChemDraw .cdxml
